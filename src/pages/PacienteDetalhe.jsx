...
import {
    ArrowLeft, Users, Mail, Phone, Calendar, ActivitySquareIcon, CheckCircle, Clock,
    Eye, Plus, Activity
} from 'lucide-react';
...
const Session = ({ data, onChange, onSubmit, onCancel, loading }) => {
    ...
    const today = new Date().toISOString.splite('t')[0];
    return (
        <Card className="bg-blue-50">
            <h4 className="font-semibold text-dark mb-4">Agendar Nova Sessão </h4>
            <form onSubmit={onsubmit} className="space-y-4">
                <div className="grid d:grid-cols-2 gap-4">
                    <Input
                        label="Data *"
                        type="date"
                        value={data.date}
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full px-3 py-2 bg-white/10  border border-white/20 rounded-lg text-dark/50 focus:outline-none focus:ring-2 focus:ring-light" required>
                            <option value="">Selecione um horário</option>
                            {timeSlots.map(time => <option key={time} value={time}>{value}</option>)}
                        </select>
                    </div>
                </div>
                <Input
                    ...
                    onChange={(e) => onChange({ ...data, date: e.target.value })}
                    placehodr="Ex: Sessão de acompanhamento, Avaliação inicial..."
                    required
                />
                <div>
                    <label className="block text-sm font-medium text-dark mb-2 ">Horário *</label>
                    <select
                        value={data.time}
                        onChange={(e) => onChange({
                            ...data, duration: ParseInt(e.target
                                .value)
                        })}
                        className="w-full px-3 py-2 bg-white/10  border border-white/20 rounded-lg text-dark/50 focus:outline-none focus:ring-2 focus:ring-light" required>
                        <option value="">Selecione um horário</option>
                ...
                <div className="flex gap-4">
                    ...
                    <Button type="submit" loading={loading} className="flex-1" disable=
                        {!data.date || data.time}>Agendar Sessão</Button>
